...
: protected 접근 제어자를 사용한다는 의미는 꼭 상속받아 사용하라는 의미이다.

cf) clone 메서드는 어디서나 사용할 수 있도록 JAVA의 가장 최상위 클래스인 Object 클래스의 메서드이지만, custom한 클래스들의 private한 속성들까지 복사할 수 있도록 상속하여 쓰도록 만들었다.

# Prototype (in JS)

js의 모든 타입에는 prototype이 존재

js에서 공유자원을 prototype이라 함.
모든 객체, 함수는 prototype 속성을 가짐.

prototype은 공유자원을 복제하여 객체로 생성하는 방법을 제공한다.

# Prototype 패턴의 핵심

1. 이 패턴은 단순히 생성 패턴 중 하나이다.

   생성의 방법으로 복사를 사용할 뿐, '복사' 키워드에 집중하지 말 것.

# Prototype 패턴이 필요한 경우

- 공유자원 : 필요할 때마다 생성해서 사용하는 것이 불가함 -> 객체는 생성되어도 자원은 복사되도록 프로토타입 패턴을 사용
